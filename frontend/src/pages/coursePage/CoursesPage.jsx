...
const CoursesPage = () => {
  ...
  const getTopicsForForm = (courseTopics) => {
    return Array.isArray(courseTopics) ? courseTopics.join(', ') : courseTopics || '';
  };
  ...
  const startCreate = () => {
    ...
    setEditing(null);
    setForm({
      title: '', description: '', content: '', week: '', batchId: '', topics: '', difficulty: ''
    });
    setMessage('');
    setError('');
    setVideoFile(null);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editing) {
        const res = await api.put(`/courses/${editing._id}`, saveData);
        courseId = res.data.course?._id || editing._id;
        setMessage(res.data.msg || 'Course updated successfully');
      } else {
        const res = await api.post('/courses/create', saveData);
  ...
  const isEditableCourse = (course) => {
    if (['TRAINER', 'HR'].includes(user?.role)) {
      return course.trainerId?._id === user?._id || user.role === 'HR';
    }
    return false;
  };
      ...
      {showCreateForm && (
        ...
        >
          <form onSubmit={handleSave} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-xs font-medium text-slate-600">Video (optional)</label>
              <input
                type="file"
                accept="video/*"
      ...
      >
        {filteredCourses.length === 0 ? (
          <div className="text-center py-12">
            <svg className="mx-auto h-16 w-16 text-slate-400 mb-4" fill="none" viewBox="0 0 24 24">
            ...
            {filteredCourses.map((course) => (
              ...
              >
                ...
                <div className="relative h-32 bg-gradient-to-br from-slate-50 to-slate-100 border-b border-slate-200">
                  ...
                  <div className="absolute top-3 right-3 rounded-md bg-emerald-100 px-2 py-1 text-[11px] font-semibold text-emerald-800">
                    {course.trainerId?.name?.split(' ')[0] || 'Trainer'}
                  </div>
                </div>
